...
router.route("/pay").post(
  ...
  asyncHandler(async (req, res) => {
    const { cartId, cartPrice, cartShippingPrice, cartTotalPrice } = req.body;
...
router.route("/finalize").get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const user = await User.findById(req.user._id);
    const checkOrder = await Order.find({'paymentResponse.pidx':String(pidx)});
